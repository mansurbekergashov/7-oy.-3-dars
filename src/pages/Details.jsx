...
export default function Details() {
  ...
  <div className="py-10">
    <div className="container mx-auto px-10">
      <Button className="mb-5" onClick={back} variant={"secondary"}> <ArrowLeft/>Back </Button>
      <div className="rounded-md shadow px-10 py-3 flex justify-between">
        <span className="inline-flex items-center gap-5">
          Status <StatusBadge status={invoice.status}/>
        </span>
        <div className="flex gap-5">
          {/* <Button variant={"secondary"}><Pen/> Edit</Button> */}
          <EditElementSheet invoice={invoice} setInvoice={setInvoice}/>
          <Button variant={"destructive"}>Delete</Button>
          {invoice.status === "pending" && <Button onClick={setPaid} variant={"outline"}>Mark as Paid</Button>}
        </div>
      </div>
